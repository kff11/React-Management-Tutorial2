...
class CustomerAdd extends React.Component {
    ...
    addCustomer = () => {
        ...
        const config = {
            headers: {
                'content-type': 'multipart/form-data' // 전달하려는 데이터에 File이 포함되어 있을 때 써야함
            }
        }
    ...
    handleFormSubmit = (e) => {
        e.preventDefault(); // 데이터가 서버에 전달됨에 있어서 오류가 있지 않도록 하는 함수
                ...
                <Dialog open={this.state.open} onClose={this.handleClose}>
                    ...
                    <DialogContent>
                            <input className={classes.hidden} accept="image/*" id="raised-button-file" type="file"
                                   file={this.state.file} value={this.state.fileName}
